...
import { Product, Dish, Order } from '@/types/restaurant';
...
interface RestaurantStore {
  ...
  addOrder: (items: Order['items']) => string;
  ...
  persist(
    (set, get) => ({
      ...
      addOrder: (items) => {
        const id = genId();
        set((s) => ({
          orders: [...s.orders, { id, items, createdAt: new Date().toISOString(), status: 'pending' }],
        }));
        return id;
      },
