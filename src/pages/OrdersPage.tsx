...
import { BulkOrderLabelsTemplate } from "@/components/BulkOrderLabelsTemplate";
...
export default function OrdersPage() {
  ...
  const {
    customers,
    createCustomer: hookCreateCustomer,
    fetchCustomers,
  } = useCustomers();
  ...
  const handleBulkPrint = useReactToPrint({
    ...
    documentTitle: `Bulk-Print-${selectedOrderIds.size}-orders`,
    onAfterPrint: () => clearSelection(),
  });
  ...
  // Fetch data on mount
  useEffect(() => {
    Promise.all([fetchOrders(), fetchProducts()]).catch((err) => {
  ...
  const handleEditOrder = (order: any) => {
    ...
    setSelectedProducts(
      Array.from(productMap.entries()).map(([productId, quantity]) => ({
        productId,
        quantity,
      })),
    );
  ...
  const handleSubmit = async () => {
    if (selectedProducts.length === 0) {
      setSubmitMessage({
        type: "error",
        text: "Adicione pelo menos um prato ao pedido",
      });
      return;
    }
    ...
    try {
      ...
      if (editingOrderId) {
        ...
        await updateOrder(editingOrderId, {
          ...
          productsRemoved:
            productsRemoved.length > 0 ? productsRemoved : undefined,
        });
        setSubmitMessage({
          type: "success",
      ...
      setSubmitMessage({
        type: "error",
        text: error.message || "Falha ao criar pedido",
      });
    }
  };
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      // Skip invalid orders
      if (!order || !order.id || !order.status || !order.created_at)
        return false;
      ...
      if (filterOrderNumber && !order.id.includes(filterOrderNumber))
        return false;
  ...
  return (
    <div className={`space-y-6 ${selectedOrderIds.size > 0 ? "pb-24" : ""}`}>
      {/* Header Section */}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-300 mt-1">
                Acompanhe os pedidos dos clientes, atualize status e gerencie as
                      ...
                      <div className="space-y-2">
                        {discountInfo && discountInfo.totalDiscount > 0 && (
                          <div className="bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg p-3 space-y-1">
                            ...
                            <div className="text-sm text-green-700 dark:text-green-400 space-y-0.5 ml-6">
                              <p>
                              ...
                              <p>
                                ...
                                {discountInfo.discountPerProduct.toFixed(2)} por
                                produto ou $
                                {discountInfo.totalDiscount.toFixed(2)} no total
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between py-3 px-4 bg-slate-100 dark:bg-slate-800 rounded-lg border-2 border-slate-300 dark:border-slate-700">
                          <div className="flex flex-1 flex-row justify-between gap-3 items-center">
                            <label className="text-lg font-semibold text-slate-700 dark:text-slate-300 block whitespace-nowrap">
                              Total do Pedido ($):
                            </label>
                            <Input
                              type="number"
                              ...
                              className="text-lg font-bold text-primary h-12 w-40"
                            />
                          </div>
                        </div>
                        <p className="text-xs text-muted-foreground">
                          Valor calculado automaticamente. Você pode editar se
                          necessário.
                        </p>
                      </div>
                    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedOrders.map((o) => {
            ...
            return (
              <Card
                key={o.id}
                className={`overflow-hidden flex flex-col transition-all ${selectedOrderIds.has(o.id) ? "ring-2 ring-primary ring-offset-2" : ""}`}
              >
                      ...
                      <button
                        ...
                        className={`flex-shrink-0 w-6 h-6 rounded flex items-center justify-center transition-colors ${
                          selectedOrderIds.has(o.id)
                            ...
                            : "border-2 border-slate-400 text-transparent hover:border-primary"
                        }`}
                        title={
                          selectedOrderIds.has(o.id)
                            ? "Deselect order"
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              showErrorToast(
                                error.message || "Falha ao atualizar pedido",
                              );
                            }
                          }}
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              showErrorToast(
                                error.message || "Falha ao finalizar pedido",
                              );
                            }
                          }}
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              showErrorToast(
                                error.message || "Falha ao cancelar pedido",
                              );
                            }
                          }}
      ...
      {/* Hidden bulk print template */}
      {selectedOrderIds.size > 0 && (
        <div style={{ display: "none" }}>
          <BulkOrderLabelsTemplate
            ref={bulkPrintRef}
      ...
      {/* Bulk print sticky action bar */}
      {selectedOrderIds.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900 text-white shadow-2xl border-t-2 border-primary">
          <div className="max-w-screen-xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
            ...
            <Button
              ...
              className="bg-primary hover:bg-primary/80 text-white font-bold px-6"
            >
              <Printer className="w-4 h-4 mr-2" />
              Imprimir pedidos ({selectedOrderIds.size})
            </Button>
          </div>
        </div>
      )}
